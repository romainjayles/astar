...
class Parser:
    ...
    def parse(self):
        ...
        complex_point_list = []
        for line in in_file:
            line_modify = line.replace(" ", "")
            simple_point_result = simple_point.findall(line_modify)
            complex_point_result = complex_point.findall(line_modify)
            simple_point_list.append(simple_point_result)
            complex_point_list.append(complex_point_result)
        self.generate_map(
            self.clear_list(simple_point_list), self.clear_list(complex_point_list))
        return self.field
        ...
        for i in range(0, self.height):
            for j in range(0, self.length):
                for obstacle_string in complex_point_list:
                    ...
                    condition_height = (
                        i < obstacle_basis[0] + obstacle_height) and (i >= obstacle_basis[0])
                    condition_length = (
                        j < obstacle_basis[1] + obstacle_length) and (j >= obstacle_basis[1])
